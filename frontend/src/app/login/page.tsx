    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted mt-1">Sign in to your PulseAI account</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-card border border-border/40 rounded-2xl p-6">
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" required className="w-full bg-secondary/50 rounded-xl px-4 py-3 pr-10 text-sm text-foreground placeholder:text-muted outline-none focus:ring-1 focus:ring-primary/40 border border-transparent focus:border-primary/30 transition-colors" placeholder="Your password" />
        ...
        <p className="text-center text-xs text-muted mt-4">
          Don&apos;t have an account? <a href="/register" className="text-primary hover:text-primary-hover transition-colors">Create one</a>
        </p>
      </div>
    </div>
  );
