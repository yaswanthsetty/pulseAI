...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  // Escape closes the drawer; Tab is trapped inside it
  const onSidebarKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === "Escape") {
      e.preventDefault();
      setMobileOpen(false);
      return;
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <aside
        ...
        className={cn(
          "flex flex-col w-55 bg-sidebar border-r border-border/50 shrink-0 transition-transform duration-200 z-50",
      ...
      <main className="flex-1 flex flex-col min-h-0">
        ...
        <header className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-border/50 bg-sidebar">
          <button
            ...
            className="p-2.5 -m-1 rounded-lg hover:bg-sidebar-hover transition-colors min-w-11 min-h-11 flex items-center justify-center"
          >
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      {theme === "dark" ? (
        <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} aria-hidden="true">
